...
myConst=constants(lower_limit=390, upper_limit=700,
                  p1=1., p2=1, wanted_TSER=.75, max_DLC_thickness=150.0)
...
'''
    ...
    LAg_thickness = random.uniform(11.0, 30.0)
    LAg=float("{0:.1f}".format(LAg_thickness))
        
    LAlN_thickness = random.uniform(20.0, myConst.max_DLC_thickness)
    LAlN=float("{0:.1f}".format(LAlN_thickness))
    
    
    child_one = (None,) + parent_one[1:5] + (LAg,) + parent_two[6:7]
    child_two = (None,) + parent_two[1:5] + parent_one[5:6] + (LAlN,)
    child_three = (None,) + parent_one[5:7] + parent_two[1:5]
    child_four = (None,) + parent_two[5:7] + parent_one[1:5]
